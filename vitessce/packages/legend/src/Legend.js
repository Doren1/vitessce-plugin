...
export default function Legend(props) {
  ...
  const visible = (visibleProp && (
    (
      !considerSelections || (
        ...
        && featureSelection.length >= 1  // ===1 에서 >=1 로 변경 (이제 plot의 color range legend가 유전자를 여러개 선택해도 뜸)
      )
    )
    || (
      isSetColor
  ...
  useEffect(() => {
    ...
    const featureSelectionLabelRaw = (
      ...
      // 수정한 부분
      ? featureSelection.map(f => featureLabelsMap?.get(f) 
        || featureLabelsMap?.get(cleanFeatureId(f)) || f).join('+')
      : null;
