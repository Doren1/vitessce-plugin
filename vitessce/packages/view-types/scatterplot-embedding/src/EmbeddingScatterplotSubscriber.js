...
export function EmbeddingScatterplotSubscriber(props) {
  ...
  // 산점도 내에서 각 점(세포)별로 유전자 발현값 가져오는 함수
  ...
  // 만약 선택된 유전자가 여러개이면 sum expression 진행
  const sumExpressionData = useMemo(()=>{
    if ( uint8ExpressionData?.length == 1 ){ // 선택된 유전자가 1개이면 해당 유전자의 발현 데이터 반환
      return uint8ExpressionData[0];
    }
    else if ( uint8ExpressionData?.length >= 2 ){  // 선택된 유전자가 2개 이상이면 모든 유전자들의 발현량 모두 합해서 반환
      return uint8ExpressionData.reduce((acc, geneData) => {
        return acc.map((value, index) => value+geneData[index]);
      });
    }
    return null;
  }, [uint8ExpressionData]);
  ...
  const sortedWeights = useMemo(() => {
    if (sumExpressionData){
      return sumExpressionData.toSorted();
    }
    return null;
  }, [uint8ExpressionData]);
    ...
    if (alignedEmbeddingData?.data) {
      const result = stratifyArrays(
        ...
        alignedEmbeddingIndex, mergedCellSets, cellSetSelection, {
          obsEmbeddingX: alignedEmbeddingData.data[0],  //x좌표데이터
          obsEmbeddingY: alignedEmbeddingData.data[1],  //y좌표데이터
          // TODO: aggregate and transform expression data if needed prior to passing here
          ...(sumExpressionData ? { featureValue: sumExpressionData } : {}),   //코드 수정
          // 원래코드   ...(uint8ExpressionData?.[0] ? { featureValue: uint8ExpressionData?.[0] } : {}),
        },
      );
      return result;
    }
    return null;
  }, [alignedEmbeddingIndex, alignedEmbeddingData, sumExpressionData,
